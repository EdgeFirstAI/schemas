...
- **Benchmark with criterion.rs**: All performance-sensitive changes need benchmarks in `crates/schemas/benches/`
...
- Integration tests in `crates/schemas/tests/` (pure Rust) and
  `crates/capi/tests/` (FFI smoke tests, C and C++ trees):
  - `crates/schemas/tests/cdr_golden.rs` — Golden CDR files generated by pycdr2 for cross-language validation
  - `crates/schemas/tests/mcap_test.rs` — Message recording/playback tests
- Benchmarks in `crates/schemas/benches/serialization.rs` using criterion.rs

**C:**
- Tests in `crates/capi/tests/c/test_*.c` using Criterion framework
...
- **Languages**:
  ...
  - C (API via FFI, C11 standard, header hand-maintained at
    `crates/capi/include/edgefirst/schemas.h`; cbindgen is declared but not invoked)
  - Python 3.8+ (bindings using pyo3; `cp38-abi3` and `cp311-abi3` wheels)
- **Build system**:
  - Cargo for Rust (lib + staticlib + cdylib)
  - Makefile for C API tests (Criterion framework)
  - maturin for Python (pyo3 extension module)
...
- `check_version_sync.sh` — Validates Cargo.toml, `crates/python/pyproject.toml`, and package.xml versions match
...
- **`crates/capi/include/edgefirst/schemas.h` is hand-maintained**: Despite
  `cbindgen` being listed as a build-dependency in `crates/capi/Cargo.toml`, it
  is not invoked by `crates/capi/build.rs`. The header is edited manually. When
  adding, removing, or modifying C API functions, update both the implementation
  (`crates/capi/src/lib.rs`, or a submodule alongside it —
  `crates/capi/src/tensor.rs` holds the tensor family, since `lib.rs` is already
  ~12k lines) and its Rust doc comments, which establish the source of truth for
  when cbindgen is eventually wired, AND
  `crates/capi/include/edgefirst/schemas.h` (the shipped header with Doxygen
  comments).
