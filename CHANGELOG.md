...
keeps its schema name but not its layout, and `DmaBuffer`/`CameraPlane` are
gone. See the Migration section below.

### Added

- **`edgefirst_msgs/Tensor`** — the unstamped tensor payload: element dtype,
  addressing grid (`shape`), byte `strides`, optional per-tensor or per-axis
  quantization, optional colorimetry, and one or more `TensorPlane`s. Carries
  `storage_kind`, `pid` and `fence_fd` once for the whole tensor, since a frame
  has a single backing store and a single acquire point.
- **`edgefirst_msgs/TensorPlane`** — one plane, in one of two exclusive
  transport modes: behind a platform handle (`handle >= 0`, with an optional
  opaque `handle_bytes` for backends whose handle is not an fd), or inline
  (`handle == -1`). A frame may not mix modes. `modifier` carries the DRM format
  modifier, so tiled and compressed layouts are describable per plane.
- **`edgefirst_msgs/TensorStamped`** — a timestamped `Tensor`, for model input
  and output topics.
- **Four-language support** for all of the above plus the redefined
  `CameraFrame`: Rust views and builders, 89 C entry points declared in
  `include/edgefirst/schemas.h`, C++ wrappers in `schemas.hpp`, and PyO3
  bindings with `.pyi` stubs.
- **Cross-language golden vectors** (`testdata/cdr/edgefirst_msgs/Tensor*.cdr`,
  `CameraFrame*.cdr`) generated by an independent `pycdr2` implementation of the
  same `.msg` contract, then decoded and re-encoded by all four surfaces.
- **Builders for all nine `mavros_msgs` buffer-backed types** (`Altitude`,
...
- **`edgefirst_msgs/CameraFrame` is redefined**, keeping its schema name — the
  topic contract — but not its layout. It is now `header` + `seq` + an embedded
  `Tensor`, byte-identical to `TensorStamped`. Width, height, format and
  colorimetry now come from the embedded tensor rather than from frame-level
  fields; dimensions read from `tensor.shape`.
- **`seq` is load-bearing for the wire layout**, not only for drop detection. It
  is a `uint64`, so it forces the embedded `Tensor` to an 8-aligned offset
  regardless of `frame_id` length. That is what makes the embedded tensor's
  bytes identical in every wrapper, and re-parseable standalone with no
  re-encoding.
...
- **`edgefirst_msgs/DmaBuffer`** — deprecated since 3.1.0. Superseded by
  `CameraFrame`, which supports multiple planes.
...
`DmaBuffer` and the old `CameraFrame`/`CameraPlane` have no compatibility shim;
the wire format changed and there is no way to read old bytes as new ones.
Producers and consumers must be updated together.
...
The plane scalars widened from `uint32` to `uint64`, and `fd` from `int32` to
`int64`, so a plane costs 48 bytes rather than 20. At 60 fps with two planes
that is roughly 3.3 KB/s of additional metadata; payloads still travel behind
handles, not inline.

To forward a camera frame's tensor onto a tensor topic, re-head it rather than
re-encoding. The layout is position-independent, so the result is byte-identical
to encoding that tensor standalone:
...
Replace `Foo::new(...)` with `Foo::builder()`:
