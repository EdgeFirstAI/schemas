...
This release replaces the `DmaBuffer` and `CameraFrame`/`CameraPlane` messages with a composed tensor family, and redefines `CameraFrame` as a thin wrapper around a shared `Tensor` payload. It is **wire-incompatible**: `CameraFrame` keeps its schema name but not its layout, and `DmaBuffer`/`CameraPlane` are gone. See the Migration section below.

### Added

- **`edgefirst_msgs/Tensor`** — the unstamped tensor payload: element dtype, addressing grid (`shape`), byte `strides`, optional per-tensor or per-axis quantization, optional colorimetry, and one or more `TensorPlane`s. Carries `storage_kind`, `pid` and `fence_fd` once for the whole tensor, since a frame has a single backing store and a single acquire point.
- **`edgefirst_msgs/TensorPlane`** — one plane, in one of two exclusive transport modes: behind a platform handle (`handle >= 0`, with an optional opaque `handle_bytes` for backends whose handle is not an fd), or inline (`handle == -1`). A frame may not mix modes. `modifier` carries the DRM format modifier, so tiled and compressed layouts are describable per plane.
- **`edgefirst_msgs/TensorStamped`** — a timestamped `Tensor`, for model input and output topics.
- **Four-language support** for all of the above plus the redefined `CameraFrame`: Rust views and builders, 89 C entry points declared in `include/edgefirst/schemas.h`, C++ wrappers in `schemas.hpp`, and PyO3 bindings with `.pyi` stubs.
- **Cross-language golden vectors** (`testdata/cdr/edgefirst_msgs/Tensor*.cdr`, `CameraFrame*.cdr`) generated by an independent `pycdr2` implementation of the same `.msg` contract, then decoded and re-encoded by all four surfaces.

### Changed

- **`edgefirst_msgs/CameraFrame` is redefined**, keeping its schema name — the topic contract — but not its layout. It is now `header` + `seq` + an embedded `Tensor`, byte-identical to `TensorStamped`. Width, height, format and colorimetry now come from the embedded tensor rather than from frame-level fields; dimensions read from `tensor.shape`.
- **`seq` is load-bearing for the wire layout**, not only for drop detection. It is a `uint64`, so it forces the embedded `Tensor` to an 8-aligned offset regardless of `frame_id` length. That is what makes the embedded tensor's bytes identical in every wrapper, and re-parseable standalone with no re-encoding.
- **PyO3 upgraded 0.26 → 0.29**, closing two advisories: an out-of-bounds read in `PyList`/`PyTuple` `nth`/`nth_back` (high) and a missing `Sync` bound on `PyCFunction::new_closure` (moderate). `numpy` (rust-numpy) moves in lockstep.

### Removed

- **`edgefirst_msgs/DmaBuffer`** — deprecated since 3.1.0. Superseded by `CameraFrame`, which supports multiple planes.
- **`edgefirst_msgs/CameraPlane`** — superseded by `TensorPlane`.
- The corresponding Rust types, C entry points, C++ wrappers, PyO3 classes, golden fixtures and benchmark cases for both.

### Migration

`DmaBuffer` and the old `CameraFrame`/`CameraPlane` have no compatibility shim; the wire format changed and there is no way to read old bytes as new ones. Producers and consumers must be updated together.

Field mapping from the old `CameraFrame`/`CameraPlane` to the new layout:

| Old | New |
| --- | --- |
| `frame.width` / `frame.height` | `frame.tensor.shape` (`[h, w]`) |
| `frame.format` | `frame.tensor.format` |
| `frame.color_*` | `frame.tensor.color_*` |
| `frame.pid` | `frame.tensor.pid` |
| `frame.fence_fd` | `frame.tensor.fence_fd` |
| `plane.fd` (`int32`) | `plane.handle` (`int64`) |
| `plane.offset` / `stride` / `size` / `used` (`uint32`) | same names, now `uint64` |
| — | `plane.modifier` — DRM format modifier; 0 = linear |
| — | `plane.handle_bytes` — opaque handle for non-fd backends |
| — | `tensor.dtype`, `tensor.strides`, `tensor.quant_*`, `tensor.storage_kind` |

`shape` is the **addressing grid, not the byte layout**: an NV12 frame carries `shape == [h, w]` with a U8 `dtype` against an `h*w*3/2` allocation. It is deliberately never validated against any buffer size. `strides` is in **bytes**, not elements.

The plane scalars widened from `uint32` to `uint64`, and `fd` from `int32` to `int64`, so a plane costs 48 bytes rather than 20. At 60 fps with two planes that is roughly 3.3 KB/s of additional metadata; payloads still travel behind handles, not inline.

To forward a camera frame's tensor onto a tensor topic, re-head it rather than re-encoding. The layout is position-independent, so the result is byte-identical to encoding that tensor standalone:

```rust
let standalone: Vec<u8> = frame.tensor().to_standalone_cdr();
```

```python
standalone: bytes = frame.tensor.to_standalone_cdr()
```
