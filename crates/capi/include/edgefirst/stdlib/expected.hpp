#ifndef EDGEFIRST_STDLIB_EXPECTED_HPP
#define EDGEFIRST_STDLIB_EXPECTED_HPP

// This file is a vendored copy of tl::expected (https://github.com/TartanLlama/expected)
// SPDX-License-Identifier: CC0-1.0
// Upstream version: 1.3.1 (latest master as of vendoring)
// Do not modify; re-vendor from upstream if an update is needed.
//
// NOTE: The upstream `tl::` namespace has been renamed to `edgefirst::stdlib::`
// to (a) avoid ODR violations if downstream code also vendors tl::expected from
// upstream and (b) unify EdgeFirst's stdlib polyfills under a single namespace.
// When C++23's `std::expected` is available, this header aliases to the standard
// library type instead of using the vendored implementation. The public API
// surface is identical; consumers use `edgefirst::schemas::expected` (defined
// in schemas.hpp) which aliases to `edgefirst::stdlib::expected`.
...
#else // fallback to vendored tl::expected
...
#define TL_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)                         \
  edgefirst::stdlib::detail::is_trivially_copy_constructible<T>
#define TL_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(T)                            \
...
// Trait for checking if a type is a edgefirst::stdlib::expected
...
#endif // EDGEFIRST_STDLIB_EXPECTED_HPP
