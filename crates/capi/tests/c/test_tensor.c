...
static edgefirst_msgs_tensor_plane_elem_t nv12_planes[2];
...
static edgefirst_msgs_tensor_builder_t *make_nv12_builder(void) {
    ...
    edgefirst_msgs_tensor_builder_set_color_space(tb, "bt709");
    edgefirst_msgs_tensor_builder_set_color_transfer(tb, "bt709");
    edgefirst_msgs_tensor_builder_set_color_encoding(tb, "bt709");
    edgefirst_msgs_tensor_builder_set_color_range(tb, "limited");
    edgefirst_msgs_tensor_builder_set_planes(tb, nv12_planes, 2);
    return tb;
}
...
Test(tensor, builder_round_trip_all_fields) {
    edgefirst_msgs_tensor_builder_t *tb = make_nv12_builder();

    uint8_t *bytes = NULL;
    size_t len = 0;
    cr_assert_eq(edgefirst_msgs_tensor_builder_build(tb, &bytes, &len), 0);
    ...
    cr_assert_str_eq(edgefirst_msgs_tensor_get_color_transfer(t), "bt709");
    cr_assert_str_eq(edgefirst_msgs_tensor_get_color_encoding(t), "bt709");
    cr_assert_str_eq(edgefirst_msgs_tensor_get_color_range(t), "limited");

    cr_assert_eq(edgefirst_msgs_tensor_get_shape_len(t), 2);
    uint64_t d = 0;
    cr_assert_eq(edgefirst_msgs_tensor_get_shape_at(t, 0, &d), 0);
    cr_assert_eq(d, 480);
    cr_assert_eq(edgefirst_msgs_tensor_get_shape_at(t, 1, &d), 0);
    cr_assert_eq(d, 640);

    cr_assert_eq(edgefirst_msgs_tensor_get_strides_len(t), 2);
    int64_t s = 0;
    cr_assert_eq(edgefirst_msgs_tensor_get_strides_at(t, 0, &s), 0);
    cr_assert_eq(s, 640);

    /* An unquantized tensor carries no scales at all. */
    size_t n = 99;
    cr_assert_null(edgefirst_msgs_tensor_get_quant_scales(t, &n));
    cr_assert_eq(n, 0);

    cr_assert_eq(edgefirst_msgs_tensor_get_num_planes(t), 2);

    edgefirst_msgs_tensor_free(t);
...
Test(tensor, plane_fields_round_trip) {
    ...
    const edgefirst_msgs_tensor_plane_t *p0 = edgefirst_msgs_tensor_get_plane(t, 0);
    cr_assert_not_null(p0);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_handle(p0), 7);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_offset(p0), 0);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_stride(p0), 640);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_size(p0), 640u * 480u);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_used(p0), 640u * 480u);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_modifier(p0), 0);
    cr_assert(!edgefirst_msgs_tensor_plane_is_inline(p0));

    size_t hb_len = 0;
    const uint8_t *hb = edgefirst_msgs_tensor_plane_get_handle_bytes(p0, &hb_len);
    ...
    size_t d_len = 1;
    const uint8_t *d = edgefirst_msgs_tensor_plane_get_data(p0, &d_len);
    cr_assert_eq(d_len, 0);
    cr_assert_null(d);

    const edgefirst_msgs_tensor_plane_t *p1 = edgefirst_msgs_tensor_get_plane(t, 1);
    cr_assert_not_null(p1);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_offset(p1), 640u * 480u);

    /* Out of range. */
    errno = 0;
    cr_assert_null(edgefirst_msgs_tensor_get_plane(t, 2));
...
Test(tensor, inline_plane_round_trip) {
    ...
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_handle(p), -1);

    size_t n = 0;
    const uint8_t *got = edgefirst_msgs_tensor_plane_get_data(p, &n);
...
Test(tensor, inline_plane_with_wrong_size_is_rejected) {
    ...
    size_t len = 0;
    errno = 0;
    cr_assert_eq(edgefirst_msgs_tensor_builder_build(tb, &bytes, &len), -1);
...
Test(tensor, camera_frame_round_trip) {
    ...
    edgefirst_msgs_camera_frame_builder_set_stamp(fb, 1234567890, 123456789);
    edgefirst_msgs_camera_frame_builder_set_frame_id(fb, "camera_0");
    edgefirst_msgs_camera_frame_builder_set_seq(fb, 99);
    cr_assert_eq(edgefirst_msgs_camera_frame_builder_set_tensor(fb, tb), 0);

    uint8_t *bytes = NULL;
    size_t len = 0;
    cr_assert_eq(edgefirst_msgs_camera_frame_builder_build(fb, &bytes, &len), 0);

    edgefirst_msgs_camera_frame_t *f = edgefirst_msgs_camera_frame_from_cdr(bytes, len);
    cr_assert_not_null(f);
    cr_assert_eq(edgefirst_msgs_camera_frame_get_stamp_sec(f), 1234567890);
    cr_assert_eq(edgefirst_msgs_camera_frame_get_stamp_nanosec(f), 123456789);
    cr_assert_str_eq(edgefirst_msgs_camera_frame_get_frame_id(f), "camera_0");
    cr_assert_eq(edgefirst_msgs_camera_frame_get_seq(f), 99);

    const edgefirst_msgs_tensor_t *t = edgefirst_msgs_camera_frame_get_tensor(f);
    cr_assert_not_null(t);
    cr_assert_str_eq(edgefirst_msgs_tensor_get_format(t), "NV12");
    cr_assert_eq(edgefirst_msgs_tensor_get_num_planes(t), 2);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_handle(edgefirst_msgs_tensor_get_plane(t, 0)), 7);

    edgefirst_msgs_camera_frame_free(f);
    edgefirst_schemas_bytes_free(bytes, len);
...
Test(tensor, builder_without_tensor_is_rejected) {
    ...
    size_t len = 0;
    errno = 0;
    cr_assert_eq(edgefirst_msgs_camera_frame_builder_build(fb, &bytes, &len), -1);
    cr_assert_eq(errno, EINVAL);
    edgefirst_msgs_camera_frame_builder_free(fb);
}
...
Test(tensor, tensor_bytes_are_identical_across_wrappers_and_frame_ids) {
    ...
    for (int i = 0; i < 3; i++) {
        /* CameraFrame */
        edgefirst_msgs_camera_frame_builder_t *fb = edgefirst_msgs_camera_frame_builder_new();
        edgefirst_msgs_camera_frame_builder_set_frame_id(fb, ids[i]);
        edgefirst_msgs_camera_frame_builder_set_seq(fb, 5);
        edgefirst_msgs_camera_frame_builder_set_tensor(fb, tb);
        uint8_t *fbytes = NULL;
        size_t flen = 0;
        cr_assert_eq(edgefirst_msgs_camera_frame_builder_build(fb, &fbytes, &flen), 0);
        edgefirst_msgs_camera_frame_t *f = edgefirst_msgs_camera_frame_from_cdr(fbytes, flen);
        cr_assert_not_null(f);
        size_t tlen = 0;
        const uint8_t *tb_bytes =
            edgefirst_msgs_tensor_get_tensor_bytes(edgefirst_msgs_camera_frame_get_tensor(f), &tlen);

        /* TensorStamped, same inputs */
        edgefirst_msgs_tensor_stamped_builder_t *sb = edgefirst_msgs_tensor_stamped_builder_new();
        edgefirst_msgs_tensor_stamped_builder_set_frame_id(sb, ids[i]);
        edgefirst_msgs_tensor_stamped_builder_set_seq(sb, 5);
        edgefirst_msgs_tensor_stamped_builder_set_tensor(sb, tb);
        uint8_t *sbytes = NULL;
        size_t slen = 0;
        cr_assert_eq(edgefirst_msgs_tensor_stamped_builder_build(sb, &sbytes, &slen), 0);
        edgefirst_msgs_tensor_stamped_t *s = edgefirst_msgs_tensor_stamped_from_cdr(sbytes, slen);
        cr_assert_not_null(s);
        size_t s_tlen = 0;
        const uint8_t *s_tbytes =
            edgefirst_msgs_tensor_get_tensor_bytes(edgefirst_msgs_tensor_stamped_get_tensor(s), &s_tlen);
        ...
        edgefirst_msgs_camera_frame_free(f);
        edgefirst_schemas_bytes_free(fbytes, flen);
        edgefirst_msgs_camera_frame_builder_free(fb);
        edgefirst_msgs_tensor_stamped_free(s);
        edgefirst_schemas_bytes_free(sbytes, slen);
        edgefirst_msgs_tensor_stamped_builder_free(sb);
    }
    free(ref);
    edgefirst_msgs_tensor_builder_free(tb);
}
...
Test(tensor, embedded_tensor_reparses_standalone) {
    ...
    size_t sa_len = 0;
    cr_assert_eq(
        edgefirst_msgs_tensor_to_standalone_cdr(edgefirst_msgs_camera_frame_get_tensor(f), &standalone, &sa_len), 0);

    edgefirst_msgs_tensor_t *t = edgefirst_msgs_tensor_from_cdr(standalone, sa_len);
    cr_assert_not_null(t);
    cr_assert_str_eq(edgefirst_msgs_tensor_get_format(t), "NV12");
    cr_assert_eq(edgefirst_msgs_tensor_get_num_planes(t), 2);
    cr_assert_eq(edgefirst_msgs_tensor_get_pid(t), 4242);
    ...
    uint8_t *direct = NULL;
    size_t direct_len = 0;
    cr_assert_eq(edgefirst_msgs_tensor_builder_build(tb, &direct, &direct_len), 0);
    cr_assert_eq(sa_len, direct_len);
    cr_assert_eq(memcmp(standalone, direct, sa_len), 0);

    edgefirst_msgs_tensor_free(t);
    edgefirst_schemas_bytes_free(standalone, sa_len);
    edgefirst_schemas_bytes_free(direct, direct_len);
    edgefirst_msgs_camera_frame_free(f);
    edgefirst_schemas_bytes_free(bytes, len);
    edgefirst_msgs_camera_frame_builder_free(fb);
    edgefirst_msgs_tensor_builder_free(tb);
...
Test(tensor, freeing_borrowed_children_is_a_safe_noop) {
    ...
    cr_assert_not_null(t);
    errno = 0;
    edgefirst_msgs_tensor_free((edgefirst_msgs_tensor_t *) t);
    cr_assert_eq(errno, EINVAL);
    ...
    const edgefirst_msgs_tensor_plane_t *p = edgefirst_msgs_tensor_get_plane(t, 0);
    errno = 0;
    edgefirst_msgs_tensor_plane_free((edgefirst_msgs_tensor_plane_t *) p);
    cr_assert_eq(errno, EINVAL);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_handle(p), 7);
...
Test(tensor, in_place_stamp_and_seq) {
    ...
    cr_assert_eq(edgefirst_msgs_camera_frame_set_stamp(bytes, len, 42, 7), 0);
    cr_assert_eq(edgefirst_msgs_camera_frame_set_seq(bytes, len, 12345), 0);

    edgefirst_msgs_camera_frame_t *f = edgefirst_msgs_camera_frame_from_cdr(bytes, len);
    cr_assert_not_null(f);
    cr_assert_eq(edgefirst_msgs_camera_frame_get_stamp_sec(f), 42);
    cr_assert_eq(edgefirst_msgs_camera_frame_get_stamp_nanosec(f), 7);
    cr_assert_eq(edgefirst_msgs_camera_frame_get_seq(f), 12345);

    /* The payload must be untouched by a header-only edit. */
    cr_assert_eq(edgefirst_msgs_tensor_get_num_planes(edgefirst_msgs_camera_frame_get_tensor(f)), 2);
    cr_assert_str_eq(edgefirst_msgs_tensor_get_format(edgefirst_msgs_camera_frame_get_tensor(f)), "NV12");
...
Test(tensor, from_cdr_null_and_invalid) {
    errno = 0;
    cr_assert_null(edgefirst_msgs_tensor_from_cdr(NULL, 10));
    cr_assert_eq(errno, EINVAL);

    uint8_t bad[] = {0xDE, 0xAD, 0xBE, 0xEF};
    errno = 0;
    cr_assert_null(edgefirst_msgs_tensor_from_cdr(bad, sizeof(bad)));
    cr_assert_eq(errno, EBADMSG);

    errno = 0;
    cr_assert_null(edgefirst_msgs_camera_frame_from_cdr(NULL, 10));
    cr_assert_eq(errno, EINVAL);

    errno = 0;
    cr_assert_null(edgefirst_msgs_tensor_stamped_from_cdr(bad, sizeof(bad)));
    cr_assert_eq(errno, EBADMSG);
}

Test(tensor, null_getters_return_defaults) {
    /* Defaults are the schema's "absent" values, not blanket zeroes. */
    cr_assert_eq(edgefirst_msgs_tensor_get_fence_fd(NULL), -1);
    cr_assert_eq(edgefirst_msgs_tensor_get_quant_axis(NULL), -2);
    cr_assert_eq(edgefirst_msgs_tensor_get_storage_kind(NULL), 0);
    cr_assert_eq(edgefirst_msgs_tensor_get_num_planes(NULL), 0);
    cr_assert_null(edgefirst_msgs_tensor_get_format(NULL));
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_handle(NULL), -1);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_size(NULL), 0);
    cr_assert(!edgefirst_msgs_tensor_plane_is_inline(NULL));

    size_t n = 99;
    cr_assert_null(edgefirst_msgs_tensor_get_quant_scales(NULL, &n));
    cr_assert_eq(n, 0);

    uint64_t d = 0;
    errno = 0;
    cr_assert_eq(edgefirst_msgs_tensor_get_shape_at(NULL, 0, &d), -1);
    cr_assert_eq(errno, EINVAL);
}

Test(tensor, free_null_is_safe) {
    edgefirst_msgs_tensor_free(NULL);
    edgefirst_msgs_tensor_plane_free(NULL);
    edgefirst_msgs_camera_frame_free(NULL);
    edgefirst_msgs_tensor_stamped_free(NULL);
    edgefirst_msgs_tensor_builder_free(NULL);
    edgefirst_msgs_camera_frame_builder_free(NULL);
    edgefirst_msgs_tensor_stamped_builder_free(NULL);
}

Test(tensor, builder_string_setters_reject_null) {
    edgefirst_msgs_tensor_builder_t *tb = edgefirst_msgs_tensor_builder_new();
    errno = 0;
    cr_assert_eq(edgefirst_msgs_tensor_builder_set_format(tb, NULL), -1);
    cr_assert_eq(errno, EINVAL);
    edgefirst_msgs_tensor_builder_free(tb);
}
...
Test(tensor, encode_into_reports_short_buffer) {
    ...
    size_t written = 0;
    errno = 0;
    cr_assert_eq(edgefirst_msgs_tensor_builder_encode_into(tb, small, sizeof(small), &written), -1);
    cr_assert_eq(errno, ENOBUFS);
    ...
    cr_assert_eq(edgefirst_msgs_tensor_builder_encode_into(tb, big, len, &written), 0);
...
Test(tensor, per_tensor_quantization_round_trip) {
    ...
    edgefirst_msgs_tensor_builder_set_quant_zero_points(tb, zero, 1);
    ...
    edgefirst_msgs_tensor_t *t = edgefirst_msgs_tensor_from_cdr(bytes, len);
    cr_assert_not_null(t);
    cr_assert_eq(edgefirst_msgs_tensor_get_quant_axis(t), -1);

    size_t n = 0;
    const float *sc = edgefirst_msgs_tensor_get_quant_scales(t, &n);
    cr_assert_eq(n, 1);
    cr_assert_float_eq(sc[0], 0.125f, 1e-6);
    const int32_t *zp = edgefirst_msgs_tensor_get_quant_zero_points(t, &n);
    cr_assert_eq(n, 1);
    cr_assert_eq(zp[0], 7);
...
Test(tensor, per_axis_quantization_round_trip) {
    ...
    size_t n = 0;
    cr_assert_not_null(edgefirst_msgs_tensor_get_quant_scales(t, &n));
    cr_assert_eq(n, 3);
...
Test(tensor, unquantized_with_scales_is_rejected) {
    ...
    edgefirst_msgs_tensor_builder_t *tb = edgefirst_msgs_tensor_builder_new();
    edgefirst_msgs_tensor_builder_set_quant_axis(tb, -2); /* unquantized... */
    edgefirst_msgs_tensor_builder_set_quant_scales(tb, scale, 1); /* ...but scales given */
...
Test(tensor, per_axis_scale_count_must_match_that_axis) {
    ...
    edgefirst_msgs_tensor_builder_set_shape(tb, shape, 2);
    edgefirst_msgs_tensor_builder_set_quant_axis(tb, 0);
    edgefirst_msgs_tensor_builder_set_quant_scales(tb, scales, 2);
...
Test(tensor, zero_points_must_match_scales_length) {
    ...
    edgefirst_msgs_tensor_builder_set_quant_axis(tb, -1);
    edgefirst_msgs_tensor_builder_set_quant_scales(tb, scale, 1);
    edgefirst_msgs_tensor_builder_set_quant_zero_points(tb, zeros, 2);
...
Test(tensor, strides_must_match_shape_rank) {
    ...
    edgefirst_msgs_tensor_builder_t *tb = edgefirst_msgs_tensor_builder_new();
    edgefirst_msgs_tensor_builder_set_shape(tb, shape, 2);
    edgefirst_msgs_tensor_builder_set_strides(tb, strides, 1);
...
Test(tensor, colorimetry_without_format_is_rejected) {
    edgefirst_msgs_tensor_builder_t *tb = edgefirst_msgs_tensor_builder_new();
    edgefirst_msgs_tensor_builder_set_color_space(tb, "bt709"); /* no format set */
...
Test(tensor_golden, tensor_decodes) {
    ...
    const edgefirst_msgs_tensor_plane_t *p = edgefirst_msgs_tensor_get_plane(t, 1);
    cr_assert_not_null(p);
    cr_assert_eq(edgefirst_msgs_tensor_plane_get_offset(p), 640u * 480u);
...
Test(tensor_golden, inline_tensor_decodes) {
    ...
    size_t n = 0;
    const uint8_t *d = edgefirst_msgs_tensor_plane_get_data(p, &n);
    cr_assert_eq(n, 8);
...
Test(tensor_golden, both_wrappers_decode_either_fixture) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        cr_assert_not_null(f);
        cr_assert_eq(edgefirst_msgs_camera_frame_get_seq(f), 99);
        cr_assert_str_eq(edgefirst_msgs_camera_frame_get_frame_id(f), "test_frame");
        cr_assert_str_eq(edgefirst_msgs_tensor_get_format(edgefirst_msgs_camera_frame_get_tensor(f)), "NV12");

        edgefirst_msgs_tensor_stamped_t *s = edgefirst_msgs_tensor_stamped_from_cdr(buf, len);
        cr_assert_not_null(s);
        cr_assert_eq(edgefirst_msgs_tensor_stamped_get_seq(s), 99);
        cr_assert_str_eq(edgefirst_msgs_tensor_stamped_get_frame_id(s), "test_frame");
...
Test(tensor_golden, long_frame_id_reheads_to_the_standalone_golden) {
    ...
    edgefirst_msgs_camera_frame_t *f = edgefirst_msgs_camera_frame_from_cdr(fbuf, flen);
    cr_assert_not_null(f);
    cr_assert_str_eq(edgefirst_msgs_camera_frame_get_frame_id(f), "a_very_long_frame_identifier_x");

    uint8_t *rehead = NULL;
    size_t rlen = 0;
    cr_assert_eq(
        edgefirst_msgs_tensor_to_standalone_cdr(edgefirst_msgs_camera_frame_get_tensor(f), &rehead, &rlen), 0);
    cr_assert_eq(rlen, slen);
    cr_assert_eq(memcmp(rehead, sbuf, rlen), 0);

    edgefirst_schemas_bytes_free(rehead, rlen);
    edgefirst_msgs_camera_frame_free(f);
    free(fbuf);
    free(sbuf);
}
