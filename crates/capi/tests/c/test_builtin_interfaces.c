 ...
 *   builtin_interfaces_time_encode(buf, cap, &written, sec, nanosec) -> int
 *   builtin_interfaces_time_decode(data, len, &sec, &nanosec) -> int
 */
...
Test(builtin_interfaces, time_encode_decode_zero) {
    ...
    int ret = builtin_interfaces_time_encode(buf, sizeof(buf), &written, 0, 0);
    cr_assert_eq(ret, 0);
...
Test(builtin_interfaces, time_encode_decode_negative) {
    ...
    int ret = builtin_interfaces_time_encode(buf, sizeof(buf), &written, -100, 500);
    cr_assert_eq(ret, 0);
...
Test(builtin_interfaces, time_encode_decode_max_nanosec) {
    ...
    int ret = builtin_interfaces_time_encode(buf, sizeof(buf), &written, 42, 999999999);
    cr_assert_eq(ret, 0);
...
Test(builtin_interfaces, time_decode_zero_length) {
    ...
    int32_t sec = 0;
    uint32_t nanosec = 0;
    int ret = builtin_interfaces_time_decode(buf, 0, &sec, &nanosec);
...
Test(builtin_interfaces, time_decode_null_out_pointers) {
    ...
    uint8_t buf[64];
    size_t written = 0;
    int ret = builtin_interfaces_time_encode(buf, sizeof(buf), &written, 10, 20);
    cr_assert_eq(ret, 0);

    // Decode with NULL output pointers -- should not crash
    ret = builtin_interfaces_time_decode(buf, written, NULL, NULL);
...
Test(builtin_interfaces, duration_encode_decode_roundtrip) {
    ...
    int ret = builtin_interfaces_duration_encode(buf, sizeof(buf), &written, 123, 456789);
    cr_assert_eq(ret, 0);
    cr_assert_gt(written, 0);

    int32_t sec = 0;
    uint32_t nanosec = 0;
    ret = builtin_interfaces_duration_decode(buf, written, &sec, &nanosec);
    cr_assert_eq(ret, 0);
...
Test(builtin_interfaces, duration_encode_decode_negative) {
    ...
    int ret = builtin_interfaces_duration_encode(buf, sizeof(buf), &written, -50, 100);
    cr_assert_eq(ret, 0);
...
Test(builtin_interfaces, duration_encode_decode_large) {
    ...
    int ret = builtin_interfaces_duration_encode(buf, sizeof(buf), &written, 300, 500000000);
    cr_assert_eq(ret, 0);
...
Test(builtin_interfaces, duration_decode_invalid_data) {
    ...
    int32_t sec = 0;
    uint32_t nanosec = 0;
    int ret = builtin_interfaces_duration_decode(bad_data, 2, &sec, &nanosec);
...
Test(builtin_interfaces, duration_decode_null_data) {
    ...
    int32_t sec = 0;
    uint32_t nanosec = 0;
    int ret = builtin_interfaces_duration_decode(NULL, 100, &sec, &nanosec);
    cr_assert_eq(ret, -1);
...
Test(builtin_interfaces, duration_encode_size_query) {
    size_t written = 0;
    int ret = builtin_interfaces_duration_encode(NULL, 0, &written, 7777, 8888);
    cr_assert_eq(ret, 0, "Size query should succeed");
    cr_assert_gt(written, 0);
}
